...
let ia_move state =
  let (mov, _) = map_fold_ac ~f:Game_ia_func.best_move ~fold:compare_func (None, worst_for (turn state)) (Game_ia_func.liste_base state (List.filter (is_valid state) (all_moves state))) in
  (* let (mov, _) = Game_ia_func.best_move state in *)
  match mov with
  | None -> assert false
  | Some m -> m
